// app/routes/app.orders.tsx - Final Corrected Version
...
// GraphQL query to fetch orders with pagination
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (statusFilter && statusFilter !== "all") {
    if (statusFilter === "paid") {
      ...
      query += ` AND financial_status:partially_paid`;
    }
  }
  ...
  try {
    ...
    const enrichedOrders = orders.map(({ node, cursor: edgeCursor }: any) => {
      ...
      // Extract custom attributes from ALL line items
      ...
      return {
        ...
        // Tour/Product info
        tourName: lineItems[0]?.node?.title || '',
        tourImage: lineItems[0]?.node?.variant?.image?.url || '',
        ...
        departureDate: customAttributes['Departure Date'] || 
                       ...
                       customAttributes['departure date'] || '',
        pickupAddress: customAttributes['Pickup Address'] || 
                       customAttributes['pickup_address'] || '',
        dropoffAddress: customAttributes['Dropoff Address'] || 
                        customAttributes['dropoff_address'] || '',
        campCategory: customAttributes['Camp Category'] || 
                      customAttributes['camp_category'] || '',
        travelers: customAttributes['Travelers'] || 
                   customAttributes['travelers'] || '',
        specialRequests: customAttributes['Special Requests'] || 
                         customAttributes['special_requests'] || '',
...
export default function OrdersPage() {
  ...
  const { orders = [], pageInfo = {}, shop } = loaderData;
  ...
  const sortedOrders = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      // Special handling for arrival date
